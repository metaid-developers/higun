...
// A rebuild on an already-running NFT mempool must not skip any step: rebuild,
// start and initialize all have to run again, in that order.
func TestNftRestartMempoolCoreRestartsStartAndInitialize(t *testing.T) {
	var steps []string
	server := &NftServer{
		mempoolInit: true,
...
func TestNftRestartMempoolCorePropagatesFailure(t *testing.T) {
	cases := []struct {
		name       string
		rebuildErr error
		startErr   error
