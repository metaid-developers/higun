...
// FtUniqueFtIncomeResponse Unique FT income response
type FtUniqueFtIncomeResponse struct {
	CodeHash string   `json:"codeHash"`
	Genesis  string   `json:"genesis"`
	Income   []string `json:"income"`
}

// FtUniqueFtSpendResponse Unique FT spend response
type FtUniqueFtSpendResponse struct {
	CodeHash string   `json:"codeHash"`
	Genesis  string   `json:"genesis"`
	Spend    []string `json:"spend"`
}
...
// FtSummaryResponse FT summary response with cursor-based pagination
type FtSummaryResponse struct {
	FtInfos    []*ft.FtInfo `json:"ftInfos"`
	Count      int          `json:"count"`
...
// FtGenesisInfoResponse FT genesis info response for single FT
type FtGenesisInfoResponse struct {
	CodeHash   string `json:"codeHash"`
	Genesis    string `json:"genesis"`
	SensibleId string `json:"sensibleId"`
	Name       string `json:"name"`
...
// FtSupplyResponse FT supply response
type FtSupplyResponse struct {
...
// FtOwnersResponse FT owners response
type FtOwnersResponse struct {
	List       []*ft.FtOwner `json:"list"`
	Total      int           `json:"total"`
...
// FtAddressHistoryResponse FT address history response
type FtAddressHistoryResponse struct {
	List       []*ft.FtAddressTx `json:"list"`
	Total      int               `json:"total"`
	Cursor     int               `json:"cursor"`
	NextCursor int               `json:"nextCursor"`
	Size       int               `json:"size"`
...
// FtGenesisHistoryResponse FT genesis history response
type FtGenesisHistoryResponse struct {
	List       []*ft.FtGenesisTx `json:"list"`
	Total      int               `json:"total"`
	Cursor     int               `json:"cursor"`
	NextCursor int               `json:"nextCursor"`
	Size       int               `json:"size"`
...
// FtOwnerTxDataResponse FT owner transaction data response
type FtOwnerTxDataResponse struct {
	CodeHash string               `json:"codeHash"`
	Genesis  string               `json:"genesis"`
	Address  string               `json:"address"`
	Income   []*ft.FtOwnerTxEntry `json:"income"`
	Spend    []*ft.FtOwnerTxEntry `json:"spend"`
}

// FtAddressHistoryDbEntryResponse FT address history db entry response
type FtAddressHistoryDbEntryResponse struct {
	TxId   string `json:"txId"`
	Time   int64  `json:"time"`
	TxType string `json:"txType"` // "income" or "outcome"
}

// FtAddressHistoryDbListResponse FT address history db list response
type FtAddressHistoryDbListResponse struct {
	Total      int                                `json:"total"`
	List       []*FtAddressHistoryDbEntryResponse `json:"list"`
	Cursor     int                                `json:"cursor"`
	NextCursor int                                `json:"nextCursor"`
	Size       int                                `json:"size"`
}
