...
func (c *Client) GetBlockByHeight(height int64) (*indexer.Block, error) {
	if c.adapter != nil {
		return c.adapter.GetBlock(height)
	}
	return nil, fmt.Errorf("chain adapter not initialized")
}
...
type txRawDetailJSON struct {
	...
	Vin           []txRawInputJSON  `json:"vin"`
	Vout          []txRawOutputJSON `json:"vout"`
}

type txRawInputJSON struct {
	Txid     string `json:"txid"`
	Vout     uint32 `json:"vout"`
	Coinbase string `json:"coinbase"`
}

type txRawOutputJSON struct {
	N            uint32                  `json:"n"`
	Value        json.Number             `json:"value"`
	ScriptPubKey txRawScriptPubKeyResult `json:"scriptPubKey"`
}

type txRawScriptPubKeyResult struct {
	Address   string   `json:"address"`
	Addresses []string `json:"addresses"`
}

func txDetailFromRawJSON(body []byte) (*TxDetail, error) {
	decoder := json.NewDecoder(bytes.NewReader(body))
	decoder.UseNumber()

	var raw txRawDetailJSON
	if err := decoder.Decode(&raw); err != nil {
		return nil, fmt.Errorf("decode raw transaction detail: %w", err)
	}
	...
	for _, out := range raw.Vout {
		satoshi, err := rpcAmountToSatoshis(out.Value)
		if err != nil {
			return nil, fmt.Errorf("parse tx output value: %w", err)
		}
		address := out.ScriptPubKey.Address
...
func NewClientWithAdapter(cfg *config.Config) (*Client, error) {
	// Use factory method to create adapter
	...
	return &Client{
		rpcClient: RpcClient, // Adapter has already set global RpcClient
		cfg:       cfg,
		params:    params,
		Rpc:       RpcClient,
		adapter:   adapter,
...
func (c *Client) getTransactionDetailByTxID(txid string) (*TxDetail, error) {
	...
	txHashParam, err := json.Marshal(txHash.String())
	if err != nil {
		return nil, fmt.Errorf("marshal transaction hash parameter: %w", err)
	}
	verboseParam, err := json.Marshal(true)
	if err != nil {
		return nil, fmt.Errorf("marshal verbose parameter: %w", err)
	}
	raw, err := c.rpcClient.RawRequest("getrawtransaction", []json.RawMessage{txHashParam, verboseParam})
	if err != nil {
...
//		return nil
//	}
func (c *Client) CheckReorg(idx *indexer.UTXOIndexer) {
	for {
		// Check if block reorganization occurred
		reorgHeight, endHeight := c.FindReorgHeight()
		// fmt.Println(">>>>>", reorgHeight, endHeight)
		if reorgHeight > 0 {
...
func (c *Client) SyncBlocks(idx *indexer.UTXOIndexer, checkInterval time.Duration, stopCh <-chan struct{}, onFirstSyncDone func()) error {
	...
	for {
		...
		// Check if block reorganization occurred
		reorgHeight, _ := c.FindReorgHeight()
		if reorgHeight > 0 {
			log.Println("find reorg !!")
			// Handle reorganization
...
// 			// Index current batch
// 			if _, _, _, err := idx.IndexBlock(convertedBlock, updateHeight); err != nil {
...
func (c *Client) ProcessBlock(idx *indexer.UTXOIndexer, height int, updateHeight bool, currentHeight int, reindex bool) error {
	...
	// fmt.Println(">>>>>", height, "transaction count:", txCount)
	// if err := msgBlock2.Deserialize(bytes.NewReader(blockBytes)); err != nil {
	// 	return fmt.Errorf("Block deserialization failed, height %d: %w", height, err)
	// }
	// for _, tx := range msgBlock2.Transactions {
	// 	fmt.Println("A==>tx", tx.TxHash())
	// 	for _, in := range tx.TxIn {
	// 		fmt.Println("   B==in", in.PreviousOutPoint)
	// 	}
	// 	for _, out := range tx.TxOut {
	// 		fmt.Println("   C==out", out.PkScript, out.Value)
	// 	}
	// }
	// 使用适配器获取区块数据 (统一格式)
	...
	// ========== 以下是旧的兼容代码 (当 adapter 为 nil 时使用) ==========
	chainName := c.cfg.RPC.Chain
	var mvcBlockMsg *bsvwire.MsgBlock
	var btcBlockMsg *wire.MsgBlock
	msgBlockInterface, txCount, expectedInTxCount, expectedOutTxCount, err := c.GetBlockMsg(chainName, int64(height))
	if txCount > 100000 {
		log.Println("find big block,txCount:", txCount, "height:", height)
	}
	if err != nil {
		errMsg := syslogs.ErrLog{
			Height:       height,
	...
	if chainName == "mvc" {
		...
		blockHash = mvcBlockMsg.BlockHash().String()
		blockTime = mvcBlockMsg.Header.Timestamp.Unix()
	} else {
		btcBlockMsg = msgBlockInterface.(*wire.MsgBlock)
		blockHash = btcBlockMsg.BlockHash().String()
		blockTime = btcBlockMsg.Header.Timestamp.Unix()
	}
	...
	startIdx := 0
	var totalInCnt, totalOutCnt, totalAddressNum int
	...
	for startIdx < txCount {
		...
		for i := startIdx; i < endIdx; i++ {
			var indexerTx *indexer.Transaction
			if chainName == "mvc" {
				tx := mvcBlockMsg.Transactions[i]
				indexerTx = c.convertMvcTxToIndexerTx(tx)
			} else {
				tx := btcBlockMsg.Transactions[i]
				indexerTx = c.convertBtcTxToIndexerTx(tx)
			}
		...
		//log.Println("---------------------Current batch:", startIdx, "to", endIdx, "transactions:", len(blockPart.Transactions), "time:", time.Since(t0).Seconds())
	...
	//增加日志
	log := syslogs.IndexerLog{
		Height:             height,
		BlockHash:          blockHash,
		ExpectedInTxCount:  expectedInTxCount,
		ExpectedOutTxCount: expectedOutTxCount,
		ActualInTxCount:    totalInCnt,
		ActualOutTxCount:   totalOutCnt,
		CompletionTime:     time.Now().Unix(),
		BlockTime:          blockTime,
		TxNum:              int64(txCount),
		AddressNum:         int64(totalAddressNum),
		//NewAddressNum:      blockPart.NewAddressNum,
	}
	syslogs.InsertIndexerLog(log)
	return nil
}
...
func (c *Client) convertMvcTxToIndexerTx(tx *bsvwire.MsgTx) *indexer.Transaction {
...
func (c *Client) convertBtcTxToIndexerTx(tx *wire.MsgTx) *indexer.Transaction {
	inputs := make([]*indexer.Input, len(tx.TxIn))
	for j, in := range tx.TxIn {
		prevTxid := in.PreviousOutPoint.Hash.String()
		vout := in.PreviousOutPoint.Index
