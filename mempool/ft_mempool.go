...
func (m *FtMempoolManager) GetMempoolUniqueFtSpendMap(codeHashGenesis string) (map[string]string, error) {
	...
	spendList, err := m.mempoolUniqueFtSpendStore.GetUniqueFtUtxoByKey(codeHashGenesis)
	if err != nil {
		return nil, err
	}
...
// GetMempoolUniqueFtIncomeMap gets income data for unique FT in mempool
func (m *FtMempoolManager) GetMempoolUniqueFtIncomeMap(codeHashGenesis string) (map[string]string, error) {
	// If codeHashGenesis is empty, get all data
	if codeHashGenesis == "" {
		// Get income data for all unique FTs
		allData, err := m.mempoolUniqueFtIncomeStore.GetAllKeyValues()
		if err != nil {
			return nil, err
		}
	...
	// Get income data for specified unique FT
	incomeList, err := m.mempoolUniqueFtIncomeStore.GetUniqueFtUtxoByKey(codeHashGenesis)
	if err != nil {
		return nil, err
	}
...
func (m *FtMempoolManager) GetMempoolGenesisUtxo(outpoint string) (utxo *common.FtUtxo, err error) {
	// Get data directly from mempool genesis utxo store using key
	data, err := m.mempoolContractFtGenesisUtxoStore.GetSimpleRecord(outpoint)
	if err != nil {
	...
	// fmt.Println("[MEMPOOL]data:", string(data))
	// Parse data: sensibleId@name@symbol@decimal@codeHash@genesis@amount@index@value{@IsSpent}
	...
	// Create FtUtxo object
	utxo = &common.FtUtxo{
		TxID:       outpointParts[0],
		Index:      parts[7], // index from value
		UtxoId:     outpoint,
		SensibleId: parts[0],
		Name:       parts[1],
		Symbol:     parts[2],
