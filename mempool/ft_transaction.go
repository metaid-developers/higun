...
type FtMempoolManager struct {
	...
	mempoolAddressFtIncomeDB *storage.SimpleDB    // Mempool income database key: outpoint+address value: CodeHash@Genesis@sensibleId@Amount@Index@Value@timestamp
	mempoolAddressFtSpendDB  *storage.SimpleDB    // Mempool spend database key: outpoint+address value: CodeHash@Genesis@sensibleId@Amount@Index@Value@timestamp@usedTxId
	...
	mempoolContractFtGenesisUtxoStore   *storage.SimpleDB    // Mempool contract genesis UTXO info database key: outpoint, value: sensibleId@name@symbol@decimal@codeHash@genesis@amount@index@value{@IsSpent}

	mempoolAddressFtIncomeValidStore *storage.SimpleDB // Mempool contract data database key: outpoint+address value: CodeHash@Genesis@sensibleId@Amount@Index@Value@timestamp
	mempoolUncheckFtOutpointStore    *storage.SimpleDB // Mempool unchecked FT outpoint database key: outpoint value: ftAddress@codeHash@genesis@sensibleId@ftAmount@index@value@timestamp
...
func (m *FtMempoolManager) HandleRawTransaction(topic string, data []byte) error {
	...
	isFtTx, err := m.processFtOutputs(tx, now)
	if err != nil {
...
func (m *FtMempoolManager) processFtOutputs(tx *wire.MsgTx, timestamp int64) (bool, error) {
	txHash := tx.TxHash().String()
	if config.GlobalConfig.RPC.Chain == "mvc" {
		txHash, _ = blockchain.GetNewHash(tx)
	}
	...
	for i, out := range tx.TxOut {
		...
		if contractTypeStr == "ft" {
			...
			// Store FT UTXO info, format: CodeHash@Genesis@sensibleId@Amount@Index@Value@timestamp
			ftAmount := strconv.FormatUint(ftInfo.Amount, 10)
			mempoolFtUtxo := common.ConcatBytesOptimized([]string{ftInfo.CodeHash, ftInfo.Genesis, ftInfo.SensibleId, ftAmount, outputIndex, valueStr, strconv.FormatInt(timestamp, 10)}, "@")
			//CodeHash@Genesis@sensibleId@Amount@Index@Value@timestamp
			...
			// ftAddress@codeHash@genesis@sensibleId@ftAmount@index@value@timestamp
			...
			// key: outpoint+ftCodehashGenesis
			//codeHash@genesis@sensibleId@customData@Index@Value
...
func (m *FtMempoolManager) processFtInputs(tx *wire.MsgTx, timestamp int64) error {
	...
	for _, in := range tx.TxIn {
		...
		if err == nil {
			...
			if len(parts) > int(in.PreviousOutPoint.Index) {
				...
				if ftUtxoContractType == "unique" {
					ftUtxoCodehashGenesis = ftUtxoCodeHash + "@" + ftUtxoGenesis
					// ftUtxoCustomData = outputInfo[4]
				}
			}
			...
			if ftUtxoCodeHash == "" || strings.Contains(ftUtxoAddress, "@") {
		...
		fmt.Printf("[MEMPOOL-input-find][%s]ftUtxoContractType: %s\n", spentUtxoID, ftUtxoContractType)
		...
		if ftUtxoContractType == "ft" {
			...
			//CodeHash@Genesis@sensibleId@Amount@Index@Value@timestamp@usedTxId
			mempoolFtUtxo := common.ConcatBytesOptimized([]string{ftUtxoCodeHash, ftUtxoGenesis, ftUtxoSensibleId, ftUtxoAmount, ftUtxoIndex, ftUtxoValue, strconv.FormatInt(timestamp, 10), txId}, "@")
			...
			fmt.Printf("[MEMPOOL-input-find][%s]recordKey: %s, mempoolUniqueUtxo: %s\n", spentUtxoID, recordKey, mempoolUniqueUtxo)
	...
	if len(allTxPoints) > 0 {
		for _, usedTxPoint := range allTxPoints {
			...
			for i, out := range tx.TxOut {
				pkScriptStr := hex.EncodeToString(out.PkScript)
				ftInfo, _, contractTypeStr, err := blockchain.ParseContractFtInfo(pkScriptStr, m.chainCfg)
				if err != nil || ftInfo == nil {
					continue
				}
				if contractTypeStr != "ft" && contractTypeStr != "unique" {
					continue
				}
	...
	for _, utxo := range incomeUtxoList {
		if utxo.ContractType == "ft" {
			...
			if len(parts) == 2 {
				...
				// Check and delete records in the new genesis output storage
