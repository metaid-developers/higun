...
func (c *ZMQClient) listen() {
	...
	for {
		...
		default:
			...
			log.Printf("ZMQ connection lost, will reconnect in %v second", config.GlobalConfig.ZmqReconnectInterval)
			time.Sleep(time.Second * time.Duration(config.GlobalConfig.ZmqReconnectInterval))
		}
	}
}
